...
function daysToYMD(d) {
  if (!d) return "0y 0m 0d";
  const y = Math.floor(d / 365), r = d - y * 365, m = Math.floor(r / 30);
  return `${y}y ${m}m ${r - m * 30}d`;
}
...
function Builder({ onAdd, draft, setDraft }) {
  const sub = SUBSTANCES.find(s => s.name === draft.substance);
  return (
    <div className="card-body">
      <div className="builder">
        <select className="select" value={draft.substance} onChange={e => setDraft({ ...draft, substance: e.target.value })}>
          {SUBSTANCES.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
        </select>
...
function ComparisonContent() {
  ...
  const computed = useMemo(() => scenarios.map(s => {
    const sub = SUBSTANCES.find(x => x.name === s.substance);
    const qtyG = (+s.qty || 0) * (UNITS[s.unit] || 1);
  ...
  return (
    <div className="shell">
      <h2 className="page-title">
        Sentence &amp; Fine Comparison <span className="subtle">— compare up to six scenarios side-by-side</span>
      </h2>
              ...
              <thead>
                <tr>
                  ...
                  {computed.map((c, i) => {
                    ...
                    return (
                      <th key={c.id} className={"col " + (isBest ? "best" : isWorst ? "worst" : "")}>
